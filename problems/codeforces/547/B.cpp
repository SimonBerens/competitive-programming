...
int main() {
    ...
    vi idxs(n);
    iota(all(idxs), 0);
    sort(all(idxs), [&](int i, int j) -> bool { return a[i] > a[j]; });
    vi res(n+1, -1);
    dsu d(n);
    F0R(j, n) {
        int i = idxs[j];
        if (i > 0 && a[i-1] >= a[i]) d.unite(i, i-1);
        if (i + 1 < n && a[i+1] >= a[i]) d.unite(i, i+1);
        smax(res[d.size(i)], a[i]);
    }
    R0F(i, n) if (res[i] == -1 || res[i] < res[i+1]) res[i] = res[i+1];
