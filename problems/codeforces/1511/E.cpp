...
template <typename T, typename C=ll>
...
void precompute() {
    // calculate the sum of the max num dominoes possible per state
    // state ...00 -> dp[i-2] + 2^(i-2+1)
    // state ....1 -> dp[i-1]
    // state ...10 -> dp[i-2]
    FOR(i, 2, pre.size()) {
        pre[i] = 2 * pre[i-2] + mi(2).pow(i-2) + pre[i-1];
    }
}
...
int main() {
    ...
    for (const auto & line: lines) {
        for (const auto [k, v] : groupby(line)) {
            if (k) continue;
            res += pre[v] * mi(2).pow(w - v);
        }
    }
    cout << res << '\n';
}