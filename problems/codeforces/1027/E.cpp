...
int main() {
    ...
    ll n, k;
    cin >> n >> k;
    // dp[i] = number of configs of size i that don't have more than m in a row
    // dp[i] = dp[i-1] + dp[i-2] + ... + dp[i-m] (dp[0] = 2)
    vmi exact(n+1), at_most(n+1);
    FOR(c, 1, n+1) {
        vector<array<mi, 2>> dp(n+1); // 0 -> no c, 1 -> yes c
        dp[0][0] = 2;
        F0R(i, n) {
            FOR(j, 1, c) {
                if (i + j > n) break;
            ...
            if (i + c <= n) dp[i+c][1] += dp[i][1] + dp[i][0];
        }
        at_most[c] = dp[n][0] + dp[n][1];
        exact[c] = dp[n][1];
    }
    mi res;
    FOR(c, 1, min(n, k)+1) {
        ll r = min(n, ceildiv(k, c)-1);
        res += exact[c] * at_most[r] / 2;
    }
    cout << res << '\n';
}